	...
	int flags, int width, int precision, int size)
{
	...
	// Check if the F_ZERO flag is set to use zero-padding
	if (flags & F_ZERO)
		padd = '0';
	...
	int flags, int width, int precision, int size)
{
	...
	// Check if F_ZERO flag is set to use zero-padding
	if ((flags & F_ZERO) && !(flags & F_MINUS))
		padd = '0';

	// Determine the extra character to be printed (sign, plus, space)
	if (is_negative)
		extra_ch = '-';
	...
	int length, char padd, char extra_c)
{
	...
	// Special case for zero precision and zero value
	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0' && width == 0)
		return (0); /* printf(".0d", 0) no char is printed */

	// For zero precision and non-zero value, replace padding character with space
	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0')
		buffer[ind] = padd = ' '; /* width is displayed with padding ' ' */

	// Add leading zeros until precision is reached
	while (prec > length)
		buffer[--ind] = '0', length++;
	...
	// Write the number with the extra character if present
	if (extra_c)
		buffer[--ind] = extra_c;
